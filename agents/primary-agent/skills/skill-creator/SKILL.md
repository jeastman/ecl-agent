...
From the your perspective, the managed project skill locations are:

| Target | Managed path | When to use it |
|---|---|---|
| Primary agent | `agents/primary-agent/skills/<skill-name>/` | The capability should be available to the main agent |
| Subagent | `agents/subagents/<role>/skills/<skill-name>/` | The capability is specific to one compiled subagent role |

Treat these as runtime-owned locations. Do not invent alternate install roots.

Use this distinction:

- **Authoring or editing a skill in-repo**: Work directly in the appropriate managed skill directory under `agents/.../skills/`.
- **Installing a staged skill into a managed location**: Use the `skill-installer` tool rather than manually copying files into place.
...
└── Bundled Resources (optional)
    ├── scripts/          - Executable code (uv Python script/Bash/etc.)
...
There are two common ways to create a new skill:
...
Run the initializer from the skill's own directory so relative paths resolve correctly.
...
Use one of the managed project skill roots:

```bash
# Primary-agent skill
scripts/init_skill.py <skill-name> --path ../../

# Subagent skill (example: coder role)
scripts/init_skill.py <skill-name> --path ../../../subagents/coder/skills
```
...
#### Option B: Manual skeleton or staged bundle

When `init_skill.py` is unnecessary, create the minimal folder structure directly:

```bash
mkdir agents/primary-agent/skills/<skill-name>
touch agents/primary-agent/skills/<skill-name>/SKILL.md
```

If the skill is being created outside the managed destination first, stage it in a workspace path, validate it, then install it with `skill-installer` into either the primary-agent scope or a specific subagent role.

Use `init_skill.py` when the skill will include bundled resources (`scripts/`, `references/`, `assets/`). Use a manual skeleton only for very small skills.
