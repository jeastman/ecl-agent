...
class TaskActionBarViewModel:
    ...
    input_suggestion: str | None
    input_history_hint: str | None
    status_message: str
    status_tone: str
...
def task_action_bar(state: AppState) -> TaskActionBarViewModel:
    ...
    return TaskActionBarViewModel(
        ...
        input_suggestion=state.task_command_suggestion,
        input_history_hint=_task_command_history_hint(state),
...
def command_palette(state: AppState) -> CommandPaletteViewModel:
    ...
    task_actions = task_action_bar(state)
    commands = [
        (
            "create_task",
            "Create task",
            "Start a new task from the current workspace",
            "New task from workspace",
            "Tasks",
            "□",
            "N",
            True,
        ...
        (
            "set_runtime_user",
            "Set runtime user",
            "Configure the per-user remote MCP runtime identity",
        ...
        (
            "resume_task",
            "Resume task",
            "Continue the currently selected paused task",
            "Continue paused selected task",
            "Tasks",
            "▶",
            "R",
            task_actions.resume_enabled,
        ),
        (
            "approve_request",
            "Approve request",
            "Open the next pending approval request",
        ...
        (
            ...
            "Begin remote MCP authorization for the selected task",
        ...
        (
            ...
            "Finish the current remote MCP authorization flow",
        ...
        (
            "open_artifacts",
            "Inspect artifacts",
            "Browse artifacts generated by recent task runs",
            "Browse runtime artifacts",
            "Inspect",
            "▤",
            "O",
            True,
        ...
        (
            "open_memory",
            "Inspect memory",
            "Open the read-only memory inspector",
        ...
        (
            "open_diagnostics",
            "View diagnostics",
            "Inspect persisted diagnostics for the selected task",
        ...
        (
            "reconnect_runtime",
            "Reconnect runtime",
            "Reconnect and recover the current operator context",
            ...
            if candidate_task_id.lower() == task_id.lower():
                items.append(
                    CommandPaletteItemViewModel(
                        ...
                        description="Jump directly to the selected task detail view",
                ...
                return CommandPaletteViewModel(
                    query=state.command_palette_query,
                    items=items,
                    result_count=1,
                    empty_message=None,
                )
    command_items: list[CommandPaletteItemViewModel] = []
    for command_id, label, description, hint, category, icon, shortcut, available in commands:
        if not available:
            continue
        label_haystack = label.lower()
        haystack = f"{label} {category} {command_id.replace('_', ' ')}".lower()
        description_haystack = f"{description} {hint}".lower()
        match_spans = _command_match_spans(query, label) if query else []
        if query and not (
            match_spans
            or _matches_command_query(query, label_haystack)
            or query in haystack
            or query in description_haystack
        ):
            continue
    ...
    if not query and state.recent_palette_commands:
        recent_ids = [command_id for command_id in state.recent_palette_commands if not command_id.startswith("open_task::")]
        by_id = {item.command_id: item for item in command_items}
        recent_items = [
            _replace_palette_category(by_id[command_id], "Recent")
            for command_id in recent_ids
            if command_id in by_id
        ]
        seen_recent = {item.command_id for item in recent_items}
        remaining_items = [item for item in command_items if item.command_id not in seen_recent]
        items = recent_items + remaining_items
    else:
        items = command_items
    ...
    return CommandPaletteViewModel(
        ...
        empty_message="No matching commands." if not items else None,
    )
...
def footer_hints(state: AppState, *, contextual: bool = False) -> _RichText:
...
def _replace_palette_category(
    item: CommandPaletteItemViewModel, category: str
) -> CommandPaletteItemViewModel:
    return CommandPaletteItemViewModel(
...
def _task_command_history_hint(state: AppState) -> str | None:
    count = len(state.task_command_history)
    if count == 0:
        return None
    if state.task_command_history_index is None:
        return f"History {count} saved"
...
def _footer_hint_labels(state: AppState, *, contextual: bool) -> list[str]:
    ...
    if state.active_screen == "task_detail":
        ...
        if state.task_detail_show_logs:
            hints[0:0] = ["Ctrl+U Page Up", "Ctrl+D Page Down", "Home Top", "End Bottom"]
        else:
            hints[0:0] = ["Ctrl+U Page Up", "Ctrl+D Page Down", "Home Top", "End Latest"]
